-- game/areas/wizard_workshop/custom.lua - hand-written. OLC never reads or
-- writes this file.
--
-- The workshop's puzzle: three potions (red, blue, green) must be poured into
-- the cauldron in that order to make a purple teleportation potion. All of that
-- is code, so all of it is here. The rooms, the items, the scenery prose and
-- the stat blocks are in rooms.lua, items.lua and mobs.lua, which OLC owns and
-- regenerates wholesale.
--
-- Two things are worth knowing about the split:
--
--   * `items` is a schema `map` field, so a patch of it **merges key by key**.
--     The laboratory's `workbench` and `magelight` scenery stay in `rooms.lua`
--     as ordinary prose; only `cauldron`, which is a function, is here.
--   * `on_drink` reaches the drinkable component because `custom.lua` is merged
--     into the flat authoring data *before* the components are built. The rest
--     of each potion's drinkable block is in `items.lua`.
--
-- `gear.lua` is gone: its ten items are in the generated `items.lua` now, which
-- is where OLC can see and edit them. It only ever existed because `items.lua`
-- had to require it for the loader to reach it.
...
return {
    rooms = {
        ...
        ["wizard_workshop.laboratory"] = {
            description = laboratory_description,
            items = { cauldron = cauldron_scenery },
            actions = {
                ...
                -- "drink" is the mudlib drink command over the drinkable
                -- component; the purple potion's `on_drink` below does the
                -- teleporting.
            },
        },
        ["wizard_workshop.pantry"] = {
            actions = {
                taste = { func = taste_pantry, hint = "taste" },
            },
        },
        ["wizard_workshop.scrying_chamber"] = {
            actions = {
                gaze = { func = gaze_scrying, hint = "gaze into the mirror" },
            },
        },
        ["wizard_workshop.archive"] = {
            actions = {
                read = { func = read_archive, hint = "read" },
                take = { func = take_vial, hint = "take vial" },
            },
        },
        ["wizard_workshop.treasure_vault"] = {
    ...
    items = {
        ["purple_potion"] = { on_drink = purple_potion_drunk },
        ["regen_draught"] = { on_drink = regen_draught_drunk },
    },
}
