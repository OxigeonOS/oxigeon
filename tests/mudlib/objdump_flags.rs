...
fn the_schema_flag_marks_what_olc_owns_and_what_it_would_drop() {
    ...
    let out = vm.command("objdump -s room:fixture.hall");
...
fn the_resolve_flag_shows_what_an_lfun_returns() {
    ...
    // `fixture.yard`'s description is a function, which is what `-r` is for.
    let plain = vm.command("objdump -s room:fixture.yard");
    assert!(
    ...
    let resolved = vm.command("objdump -s -r room:fixture.yard");
    assert!(
...
fn the_depth_flag_goes_deeper_and_is_bounded() {
    ...
    // `fixture_cloak` nests three deep: item -> armour -> resist. That is where
    // the default runs out, and it is the shape every component has.
    let shallow = vm.command("objdump -d 1 template:fixture_cloak");
    let deep = vm.command("objdump -d 4 template:fixture_cloak");
    assert!(
    ...
    let clamped = vm.command("objdump -d 99 template:fixture_cloak");
    assert!(clamped.contains("Raw fields"), "{clamped}");

    // A malformed flag says so rather than being read as a spec.
    assert!(vm.command("objdump -d template:fixture_cloak").contains("needs a depth"));
...
fn the_inherit_flag_shows_the_metatable_chain_without_drowning_it() {
    ...
    let plain = vm.command("objdump template:fixture_stone");
    assert!(!plain.contains("Inherited"), "{plain}");

    let out = vm.command("objdump -i template:fixture_stone");
...
fn the_all_flag_turns_everything_on() {
    ...
    let out = vm.command("objdump -a room:fixture.yard");
...
fn traits_are_still_read_through_the_daemon() {
    ...
    let out = vm.command("objdump fixtureuser");
