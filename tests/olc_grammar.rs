...
///
/// This is what keeps a hand-authored area safe: thornhollow's rooms carry
/// inline action functions, and a regeneration would delete them.
#[test]
fn an_unmanaged_area_is_refused_and_names_adopt() {
    let mut vm = RealVm::boot_with_fixture_world(0);

    let out = vm.command("olc thornhollow");
    assert!(out.contains("not"), "{out}");
    assert!(
        out.contains("olc adopt thornhollow") || out.contains("_meta.lua"),
