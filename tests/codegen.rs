...
/// The gear file is loaded, which discovery-by-name would have dropped.
///
/// `wizard_workshop` has both `items.lua` and `gear.lua`, and only five names
/// are entry points. Rather than teach the loader a special case, `items.lua`
/// appends `gear.lua` — one convention instead of one exception.
#[test]
fn an_areas_extra_files_are_reached_through_its_entry_file() {
    let mut vm = RealVm::boot_real_mudlib_with_probe();

    assert_eq!(
        vm.eval("return tostring(DAEMON.items.get('apprentice_dagger') ~= nil)").unwrap(),
        "true",
        "gear.lua was not loaded — discovery dropped it"
    );
}
