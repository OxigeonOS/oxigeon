...
function M.use(user, id, opts)
    ...
    -- ─── Cooling is measured here and refused further down ───────────────────
    --
    -- It used to refuse on the spot, because the rule was that **only roundtime
    -- enqueues**: a cooldown said "not this, for a while" and roundtime said
    -- "not yet, but soon and certainly".
    --
    -- That is a true distinction and it is not one the player is in a position
    -- to make. From the seat, "Not yet. (1s)" and "You will emberlance next"
    -- are the same situation — *I want to do this and it is not happening* —
    -- and getting two different behaviours out of one intent reads as the game
    -- being arbitrary. So a cooldown queues too.
    --
    -- The refusal moves below target resolution rather than the enqueue moving
    -- up, because the reason the enqueue is down there still holds: a mistyped
    -- target must refuse immediately rather than queue something doomed.
    local cooling, cool_left = false, 0
    if DAEMON and DAEMON.cooldown then
        ...
        if spec.cooldown.seconds > 0 and not DAEMON.cooldown.ready(user, key) then
            cooling = true
            cool_left = DAEMON.cooldown.remaining(user, key)
        end
        if spec.gcd and not DAEMON.cooldown.ready(user, GCD_KEY) then
            cooling = true
            cool_left = math.max(cool_left, DAEMON.cooldown.remaining(user, GCD_KEY))
        end
    end
    ...
    -- **Waiting enqueues; being unable refuses.** Roundtime and a cooldown are
    -- both waiting, so both queue. A cost you cannot pay, a requirement you do
    -- not meet, an unknown ability and a mistyped target are all *unable*, and
    -- all still refuse — queueing those would be promising something that
    -- cannot happen.
    --
    ...
    if spec.track and not (opts.from_queue) and DAEMON and DAEMON.queue then
        local waiting = DAEMON.queue.in_roundtime(user, spec.track)
        if waiting or cooling then
            ...
            -- Whichever gate clears last is when it will actually happen.
            local left = math.ceil(math.max(
                waiting and DAEMON.queue.roundtime(user, spec.track) or 0,
                cool_left))
    ...
    -- Reached when there is no track to queue on, or when the queue is
    -- replaying this and the cooldown has not cleared yet. The third return is
    -- the machine-readable reason: `queue_d` puts a `"cooldown"` entry back
    -- rather than dropping it, because a cooldown ends and a bad target does
    -- not. A caller matching on the *message* would be matching on prose.
    if cooling then
        return false, "Not yet. (" .. math.ceil(cool_left) .. "s)", "cooldown"
    end
