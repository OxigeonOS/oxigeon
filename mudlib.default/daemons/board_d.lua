-- mudlib/daemons/board_d.lua — A notice board, and the first real consumer of
...
--
-- ─── Mudlib, because the board is a mechanism ───────────────────────────────
--
-- This lived in the game layer on the argument that "a board is content". That
-- conflates the board's *contents* — which are in the database — with the board
-- *mechanism*, which is what this file is: uuid prefix resolution, atomic view
-- counting, recursive-merge editing, two-query `or` emulation and an expiry
-- sweep. None of that is one game's.
--
-- The tell is that its own test builds a poster out of a bare table and touches
-- no room, item or creature: there is no world in here to be content about.
-- `channel_d` is the same shape and was always on this side of the line — a
-- mechanism whose *instances* the game declares.
--
-- What genuinely is the game's — which categories exist and how long a notice
-- lives — is `M.configure`, called from the game's init beside
-- `DAEMON.channel.create`.
--
...
--- nothing. Which ones exist is the game's; that there is a closed list is not.
...
--- What this board is for, from the game layer.
---
--- Both fields are optional and both keep their default when not given, so a
--- game that never calls this gets a working board — the same contract every
--- other configurable thing in the mudlib has.
--- @param opts table { categories = { "..." }, lifetime = seconds }
--- @return boolean
function M.configure(opts)
    if type(opts) ~= "table" then return false end

    if type(opts.categories) == "table" and #opts.categories > 0 then
        local out = {}
        for _, c in ipairs(opts.categories) do
            if type(c) == "string" and c ~= "" then out[#out + 1] = c end
        end
        if #out > 0 then M.CATEGORIES = out end
    end

    local lifetime = tonumber(opts.lifetime)
    if lifetime and lifetime > 0 then LIFETIME = lifetime end
    return true
end
