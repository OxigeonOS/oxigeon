...
---
--- ─── A resolver may ask to be put back ───────────────────────────────────────
---
--- `resolve` returns `"retry"` for "not yet, but this entry is still good" —
--- which is what a cooldown is. Anything else falsey drops the entry.
---
--- The distinction has to be made by the resolver rather than here, because
--- only it knows why it failed: a target that walked away is gone for good and
--- a cooldown is over in two seconds. Without it, letting a cooldown enqueue
--- would mean the entry was popped, refused, and silently lost — the player
--- queues an ability and nothing ever happens.
---
--- Bounded by the staleness rule already above, so an entry that can never run
--- is dropped rather than blocking the queue head for ever.
--- @return boolean
function M.advance(entity, track)
    ...
    -- Put back at the head, not the tail: the player asked for this first and
    -- a cooldown is not a reason to reorder their intentions.
    if ran == "retry" then
        table.insert(held.queue, 1, entry)
        return false
    end
