...
M.fields = {
    ...
    -- The creature's own attack rate, for one with nothing in its hands: how
    -- many bites fit in one of its rounds. Beside `damage` because the two
    -- together are a dps statement, exactly as a weapon's are.
    --
    -- Without this a creature's rate came only from `round_length`, which moves
    -- 0.05s per point of dexterity — so a rat and a bear swung at very nearly
    -- the same speed and no amount of authoring could separate them.
    { name = "speed", type = "number", default = 1.0, min = 0.1, max = 5,
      editable = true,
      help = "Attacks per round with no weapon. 2.0 is twice as often." },
