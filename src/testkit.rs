...
use crate::core::auth::AuthWorker;
use crate::core::compute::ComputeBridge;
use crate::core::logging::GameLogger;
use crate::core::scripting::debugger::{DebugState, SharedDebugState};
use crate::core::scripting::efuns::EfunContext;
use crate::core::scripting::{LuaCommand, ScriptEngine};
use crate::core::session::{Session, SessionHandler, SessionOutput};
...
impl RealVm {
    ...
    ///
    /// Same shape as [`RealVm::boot_real_mudlib_with_probe_opts`]: the game is
    /// **copied** into a temp root and its `init.lua` moved aside, so the probe
    /// is what the engine loads and still hands off to the real one. Copying is
    /// what makes `require` and `list_dir` agree about where the game is, and
    /// what makes it safe for a test to write into the game root.
    ///
    /// Nobody logs in and the real dispatcher is not exercised — use
    /// [`RealVm::boot_roots_playing`] for that. Use this to ask what the content
    /// *does*.
    ///
    /// `game_path` and `log_dir` on `opts` are overwritten; everything else is
    /// honoured.
    pub fn boot_roots_with_probe(
        mudlib: &std::path::Path,
        game: &std::path::Path,
        mut opts: TestCtx,
    ) -> Self {
        let logs = TempDir::new().unwrap();
        let game_copy = TempDir::new().unwrap();

        copy_dir(game, game_copy.path());
        std::fs::rename(
            game_copy.path().join("init.lua"),
            game_copy.path().join("real_init.lua"),
        )
        .expect("the game root has no init.lua");
        std::fs::write(
            game_copy.path().join("init.lua"),
            PROBE_GAME_LAYER.replace("{probe}", PROBE_DISPATCHER),
        )
        ...
        opts.game_path = Some(game_copy.path().to_path_buf());
        opts.log_dir = Some(logs.path().to_path_buf());

        let mut vm = Self::boot_inner_at(None, mudlib.to_path_buf(), opts);
        vm._logs = Some(logs);
        vm._game = Some(game_copy);
        assert_eq!(vm.eval("return 'ready'").unwrap(), "ready");
        vm
    }

    /// The same pair of roots, with a logged-in playing session.
    ///
    /// For the tests that are genuinely about what a player sees: a rendered
    /// listing, a refusal's wording, what a verb does to a session. `command`
    /// works; `eval` does not, because the real dispatcher is `on_input`.
    ///
    /// [`RealVm::lua`] does work, through a `fixtureeval` command written into
    /// the copied game tree — so a test can still read state, at the cost of
    /// going through the dispatcher to do it. It exists only in the copy:
    /// adding an eval verb to a real game to make tests easier would be putting
    /// a hole in the shipped game for the convenience of the suite.
    ///
    /// `opts.start_room` is honoured and usually has to be set, since the
    /// caller's world decides where a character appears.
    ///
    /// `chargen` is the keystrokes that get a fresh account from the character
    /// selector into the world — empty for a game that has none. See
    /// [`RealVm::login_as_with`].
    pub fn boot_roots_playing(
        mudlib: &std::path::Path,
        game: &std::path::Path,
        mut opts: TestCtx,
        chargen: &[&str],
        ...
        copy_dir(game, game_copy.path());
        std::fs::create_dir_all(game_copy.path().join("cmds")).unwrap();
        std::fs::write(game_copy.path().join("cmds/fixtureeval.lua"), FIXTURE_EVAL).unwrap();
        ...
        vm._logs = Some(logs);
        vm._game = Some(game_copy);
        vm.login_as_with("testkituser", "a good long testkit password", chargen);
        vm
    }
...
{PROBE_DISPATCHER}"),
    ...
    fn login_as(&mut self, user: &str, password: &str) {
        self.login_as_with(user, password, &[])
    }

    /// The same, for a game that puts something between authentication and the
    /// world.
    ///
    /// `mudlib/login.lua` hands off to `game/character_select.lua` when the game
    /// ships one, and from there a new account has to choose or build a
    /// character before `enter_game` ever runs. That flow is *content* — how
    /// many characters an account may keep, whether creation is a menu or a
    /// wizard, what it asks — so the harness cannot know it. `steps` is the
    /// keystrokes that get a fresh account into the world, fed one at a time
    /// until the game says "Welcome".
    ///
    /// Empty for a game with no selector, which is the shipped demo, so
    /// [`RealVm::login_as`] is this with nothing to say.
    fn login_as_with(&mut self, user: &str, password: &str, steps: &[&str]) {
        ...
        // **Everything the banner and the two earlier steps produced, gone.**
        //
        // The wait below has to be able to tell the *reply to the password*
        // from what was already in the channel, and it cannot: the connect
        // banner says "Welcome to <game>", which is the word this used to wait
        // for. Under load the earlier `drain_for` calls give up before the
        // banner arrives, it lands here instead, and the login declares itself
        // finished before the account has been created — after which every
        // keystroke is one screen out of place and the failure reads as a
        // broken character-creation script.
        ...
        // ─── What "logged in" actually looks like ───────────────────────────
        //
        // A **prompt**. `send_prompt` produces a `Raw`, and only
        // `commands.dispatch` and `enter_game` call it — a login screen and a
        // character menu both write with `send`, which does not. So a prompt
        // means the command dispatcher is on the other end, and nothing else
        // does.
        //
        // Prose cannot answer this. A game with a character selector greets the
        // *account* with "Welcome, <user>." and then shows a menu, so waiting
        // for "Welcome" left the session sitting at "Your choice:" believing it
        // was in the world.
        //
        // The Argon2 hash means silence for a few hundred milliseconds first,
        // which is why this waits for text before it will accept quiet as an
        // answer.
        // Silence only means "finished" once the account exists. Before that it
        // is the hash, and the two are indistinguishable from out here — which
        // is why this names the one line the mudlib prints when the worker comes
        // back. That is this repository's own login talking to this
        // repository's own harness, not a game's prose: `login_as` types `new`,
        // so it already knows which path it is on.
        //
        // And quiet is only an answer at all when the caller *has* steps. With
        // none, the only acceptable end is the prompt: the demo prints "Welcome
        // back" and then a room description and then prompts, and treating the
        // gap in the middle as the end declared the login finished one screen
        // early.
        let (prompted, out) = self.drain_to_prompt_after(
            |t| t.contains("Account created") || t.contains("Welcome back"),
            if steps.is_empty() { None } else { Some(Duration::from_millis(400)) },
            Duration::from_secs(45),
        );

        if prompted {
            // Straight into the world: a game with no selector, which is the
            // shipped demo. Anything the caller had to say about character
        ...
        if !steps.is_empty() {
            ...
            for step in steps {
                ...
                // Each key on its own: a menu that redraws is one exchange per
                // keystroke, and sending the next one into a screen that has
                // not been drawn yet is how a chargen script desynchronises.
                let (prompted, screen) = self.drain_to_prompt_text(Duration::from_secs(10));
                transcript.push_str("\n>>> ");
                transcript.push_str(step);
                transcript.push('\n');
            ...
            // The steps ran out and nothing is dispatching commands. The whole
            // transcript, because the answer is always "the script no longer
            // matches the flow" and only the transcript says which step.
            panic!("the character-creation steps did not reach the world:\n{transcript}");
        }

        // No prompt and nothing to say. Either the login failed — a bounce back
        // to the username prompt looks exactly like this — or the game has a
        // character selector and the caller supplied no steps for it. The
        // transcript distinguishes them, which a "login did not reach the game"
        // on its own would not.
        panic!("login did not reach the game; output was:\n{out}");
    }
    ...
    fn drain_to_prompt(&mut self, timeout: Duration) -> bool {
        self.drain_to_prompt_text(timeout).0
    }

    /// Collect until the session has been silent for `quiet`, or `cap` elapses.
    ///
    /// [`RealVm::drain_for`] gives up after 20ms of silence, which is right for
    /// one command and wrong for a login: account creation hands off to the
    /// Argon2 worker and comes back in bursts with real gaps between them, so a
    /// 20ms rule returns in the middle and leaves the rest of the login in the
    /// channel to be misread as the next step's reply.
    fn drain_until_quiet(&mut self, quiet: Duration, cap: Duration) -> String {
        ...
        while Instant::now() < deadline {
            match self.output.try_recv() {
                ...
                Ok(_) => silent_since = Instant::now(),
                Err(mpsc::error::TryRecvError::Empty) => {
                    // **Quiet only counts once something has been said.**
                    // Silence before the reply starts is not the end of the
                    // reply, and on the login path that silence is the Argon2
                    // hash — a few hundred milliseconds on an idle box and
                    // seconds on a loaded one. Breaking on it returned before
                    // the account existed, so the next keystroke was swallowed
                    // by a login that had not finished and every step after it
                    // was one screen out of place.
                    if !text.is_empty() && silent_since.elapsed() >= quiet {
                        break;
                    }
                    Self::wait_step(silent_since);
                }
    ...
    /// The same, keeping what came past on the way, and giving up as soon as
    /// the session goes quiet rather than waiting out the whole window.
    ///
    /// Both halves matter for the caller that uses it. A character-creation
    /// script is diagnosed entirely from the screen it stopped on, so the text
    /// has to survive; and a screen that is *not* a prompt is the ordinary case
    /// on every step but the last, so waiting the full timeout for each of them
    /// put twenty-odd seconds of dead air into every playing boot.
    fn drain_to_prompt_text(&mut self, timeout: Duration) -> (bool, String) {
        self.drain_to_prompt_after(|_| true, Some(Duration::from_millis(400)), timeout)
    }

    /// The same, with a say in when silence is allowed to mean "done".
    ///
    /// ─── Why quiet is not enough on its own ─────────────────────────────────
    ///
    /// The login prints "Creating your account…", hands the password to the
    /// Argon2 worker, and says nothing for a few hundred milliseconds. Text
    /// followed by silence is exactly what a finished screen looks like, so a
    /// plain quiet rule declares the login over in the middle of the hash — and
    /// the next keystroke lands in a login that has not finished.
    ///
    /// `armed` is the caller saying which text means the reply has genuinely
    /// begun. Until it holds, only a prompt or the deadline ends the wait.
    fn drain_to_prompt_after(
        &mut self,
        ...
        loop {
            match self.output.try_recv() {
                Ok(SessionOutput::Raw(_)) => return (true, text),
                Ok(SessionOutput::Text(t)) => {
                    text.push_str(&t);
                    silent_since = Instant::now();
                }
                ...
                Err(mpsc::error::TryRecvError::Disconnected) => return (false, text),
            }
        }
    }
...
pub fn efun_context(
    ...
    let document_store = Arc::new(
        crate::domain::models::DieselDocumentStore::new(pool.clone(), opts.documents.clone())
